...
// GxEPD2_AVR_BW display(GxEPD2::GDEP015OC1, /*CS=*/ SS, /*DC=*/ 8, /*RST=*/ 9,
// /*BUSY=*/ 7);
...
#ifdef DBG
#define FIVE_MIN 1   // 30 number of cycles during 5 minutes
#define TWENTY_MIN 4 // 120 number of cycles during 20 min
#define TWO_HOUR 24  // 720 number of cycles during 2 hours
#else
...
Settings settings{PIN_DHT, TRANSISTOR_SWITCH_PIN,
#ifdef DBG
                  development
#else
                  production
...
void loop() {
  ...
  while (sleepCnt <= FIVE_MIN) {
    ...
    // Turn of Brown Out Detection (low voltage). This is automatically
    // re-enabled upon timer interrupt
    // sleep_bod_disable();

    // Ensure we can wake up again by first disabling interrupts (temporarily)
    // so the wakeISR does not run before we are asleep and then prevent
    // interrupts, and then defining the ISR (Interrupt Service Routine) to run
    ...
    WDTCSR = bit(WDIE) | bit(WDP3) | bit(WDP2) | bit(WDP1) |
             bit(WDP0); // set WDIE, and 1 second delay
    wdt_reset();
  ...
  // Prevent sleep mode, so we don't enter it again, except deliberately, by
  // code
  // sleep_disable();
...
ISR(WDT_vect) {
  ...
  // Turn off watchdog, we don't want it to do anything (like resetting this
  // sketch)
  wdt_disable();
